    ...
    withMethods(
        (
            ...
            async loadCharityBySlug(charitySlug: string) {
                patchState(store, { loading: true });
                const charity = await client.getCharityBySlugPromise(
                    charitySlug,
                );
                patchState(store, { selectedCharity: charity, loading: false });
            },
            async loadCampaignBySlug(
                charitySlug: string,
                campaignSlug: string,
            ) {
                patchState(store, { loading: true });
                const campaign = await client.getCampaignBySlugPromise(
                    charitySlug,
                    campaignSlug,
                );
                patchState(store, {
                    selectedCampaign: campaign.campaign,
                    selectedCharity: campaign.charity,
                    loading: false,
                });
            },
            ...
            addItemToCart(product: CartProduct) {
                ...
                const currentProduct = cartItems[product.slug];
                patchState(store, {
                    cartItems: {
                        ...cartItems,
                        [product.slug]: {
            ...
            updateCartProduct(product: CartProduct) {
                const cartItems = store.cartItems();
                patchState(store, {
                    cartItems: { ...cartItems, [product.slug]: product },
                });
            },
            deleteItemFromCart(productSlug: string) {
                const { [productSlug]: removedProperty, ...remainingObject } =
                    store.cartItems();
